...
tr %>% ggplot(aes(parent_category_name,y=deal_probability))+
  ...
  theme(plot.title = element_text(hjust = 0.5))
#price 
pricena = which(is.na(tr$price))
#補平均值
#tr$price[pricena] = mean(tr$price,na.rm=TRUE)
#取log1p = log(1+x)
...
svd_all <- svd_all[,c(2,3,18:36)]
train_bind <- tr_te %>% left_join(svd_all,by=c("item_id"="item_id","user_id"="user_id"))
```

```{r}
cat("Preparing data...\n")
X <- tr_te %>% 
  select(-txt,-item_id,user_id) %>% 
...
xgb_train_1 <- train(x=dtrain, trControl = xgb_trcontrol_1,tuneGrid = xgb_grid_1, method = "xgbTree")
